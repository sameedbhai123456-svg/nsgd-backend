...
// ========== PAGES ==========
const allowedPages = ['dashboard.html', 'entry.html', 'ledger.html', 'customer.html', 'expenses.html', 'analysis.html', 'index.html', 'login.html'];
...
app.post('/api/save-entry', auth, async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      data.name = data.name || data.customerName || data.customer || 'Unnamed';
      data.contact = data.contact || data.customerContact || '';
    }
